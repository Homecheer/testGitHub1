...
<table width="100%"  border="0" align="center" cellspacing="1">
 ...
 <c:forEach items="${aList}" var="al">
  <c:if test="${(al.payType eq '预付款到发货' && al.status eq '2') || (al.payType eq '预付款到发货' && al.status eq '1')}" >
  <tr>
    <td align="center"><a href="SoItem?soid=${al.soid }">${al.soid }</a></td>
    <td align="center">${al.createTime }</td>
	<td align="center">${al.customerName }</td>
	<td align="center">${al.userName }</td>
    ...
    <td align="center"><c:if test="${al.status eq '1'}">未预收</c:if><c:if test="${al.status eq '2'}">已到货</c:if></td>
