...
<table width="100%"  border="0" align="center" cellspacing="1">
 ...
 <c:forEach items="${aList}" var="al">
  ...
  <tr>
    ...
    <td align="center"><c:if test="${al.status eq '2' && al.payType eq '货到付款'}">未付款</c:if><c:if test="${al.status eq '1' && al.payType eq '款到发货'}">未付款</c:if><c:if test="${al.status eq '1' && al.payType eq '预付款到发货'}">未预付</c:if><c:if test="${al.status eq '2' && al.payType eq '预付款到发货'}">已到货,可收款</c:if></td>
